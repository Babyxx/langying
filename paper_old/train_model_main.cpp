...
void init()
{
    ...
    finIdfPT.close();
    //for(std::map<int,double>::iterator iter = mapIdfPT.begin();iter != mapIdfPT.end();iter++)
...
int main ( int argc, char * argv[] )
{
    ...
    */
    ...
    //svm_node featureArray[126][271432];
    //prob.x = new svm_node[126][271432];
    ...
    while (NULL != (ent = readdir(pDir))){
        if (ent->d_type == 8){
            ...
            extractFeature(pos_name, prob.l, featureArray);
            ...
            //std::cout<<file_name<<"  "<<prob.y[i]<<std::endl;
            i++;
        }
    }
